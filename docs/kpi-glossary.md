...
|---|---|---|
| Units | Items sold (returns are kept in a separate column) | `SUM(Sales[Units])` |
...
| Promo Uplift % | Extra units on promotion days vs. the normal rate | `DIVIDE([Units] - [Baseline Units], [Baseline Units])` over promotion rows that have a baseline |
| Post-promo Dip % | Sales lost in the 7 days after a promotion | Same formula over post-promotion rows that have a baseline |
